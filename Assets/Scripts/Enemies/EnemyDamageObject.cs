...
public class EnemyDamageObject : MonoBehaviour
{
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        {
            if (transform.CompareTag("EnemySkill"))
            {
                DamageX(other);
            } else if (!_playerGameObject.GetComponent<Animator>().GetCurrentAnimatorStateInfo(0).IsName("DefaultBlockSkill001") && !_playerGameObject.GetComponent<Animator>().GetCurrentAnimatorStateInfo(1).IsName("DefaultBlockSkill001") && !_playerGameObject.GetComponent<Animator>().GetCurrentAnimatorStateInfo(2).IsName("DefaultBlockSkill001"))
            {
                DamageX(other);
            }
        }
    ...
    private void DamageX(Collider other)
    {
