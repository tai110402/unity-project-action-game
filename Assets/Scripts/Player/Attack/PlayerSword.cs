...
public class PlayerSword : MonoBehaviour, IPlayerWeapon
{
    ...
    private void UpdateSkill()
    {
        ...
        {
            if (usingSkillArray[i].AnimationName.Contains("FirstSwordSkill"))
            {
                _firstSkill = usingSkillArray[i];
            }
            if (usingSkillArray[i].AnimationName.Contains("SecondSwordSkill"))
            {
                _secondSkill = usingSkillArray[i];
            }
            if (usingSkillArray[i].AnimationName.Contains("ThirdSwordSkill"))
            {
                _thirdSkill = usingSkillArray[i];
            }
            if (usingSkillArray[i].AnimationName.Contains("DefaultFirstNormalSwordSkill"))
            {
                _firstNormalSkill = usingSkillArray[i];
            }
            if (usingSkillArray[i].AnimationName.Contains("DefaultSecondNormalSwordSkill"))
            {
    ...
    {
        Debug.Log("Sword");
        for (int i = _indexOfSwordAnimatorLayer; i < _playerAnimator.layerCount; i++)
        {
            _playerAnimator.SetLayerWeight(i, 0f);
        }
        _playerAnimator.SetLayerWeight(_indexOfSwordAnimatorLayer, 1f);
    }
    public float Attack(Skill skill, float skillStartTime)
    {
        ...
        {
            if (skill.AnimationName == "DefaultFirstNormalSwordSkill" || skill.AnimationName == "DefaultSecondNormalSwordSkill")
            {
